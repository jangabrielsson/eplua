...
class LuaEngine:
    ...
    async def load_and_run_file(self, file_path: Union[str, Path], script_name: Optional[str] = None):
        """
        Load and run a Lua file.
        
        Args:
            file_path: Path to the Lua file to load and run
            script_name: Optional name for the script (for debugging)
        """
        file_path = Path(file_path)
        ...
        try:
            ...
            raise RuntimeError(f"Failed to read Lua file {file_path}: {e}")
            
        script_name = script_name or file_path.name
        await self.run_script(lua_code, script_name)
        
    async def run_until_complete(self, lua_code: str, script_name: Optional[str] = None):
        """
        Run Lua code and keep the engine running until all timers complete.
        
        This is useful for scripts that set up timers and need to keep running
        until those timers finish.
        ...
        # Keep running while there are active timers
        while self._timer_manager.get_timer_count() > 0 and self._running:
            await asyncio.sleep(0.1)
            
        logger.info("All timers completed, stopping engine")
        await self.stop()
